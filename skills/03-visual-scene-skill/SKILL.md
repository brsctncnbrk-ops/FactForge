...
- Human doc: `/templates/template-catalog.md` (12 templates: purpose, props, examples, 9:16 notes).
- NEVER copy schemas or catalog content into this folder — reference the paths above.

## The 12 templates

map-scene, timeline-scene, stat-card, comparison-split, list-reveal, quote-card, silhouette-scene, chart-scene, icon-grid, text-emphasis, image-spotlight, transition-break. Nothing else exists. Read `references/template-selection.md` when mapping scenes to templates.
...
3. `template` must be one of the 12; `props` must satisfy that template's schema (required props, allowed values).
